...
- Enforce complexity guardrails to avoid: preset-heavy, implementation-heavy, too-many-defaults, over-hard-validation, and scattered-constraints drift.
...
- mandatory `Complexity Guardrails` section covering the five anti-bloat checks (预设偏多 / 实现偏重 / 默认行为太多 / 校验过硬 / 约束分散),
